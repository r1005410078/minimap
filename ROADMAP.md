...
- **下一步**：第一阶段收尾先推进「布局切换动画」；同时仍需完成「视口锚点稳定」，两项都到位后再把第一阶段勾完（按 brainstorm → spec → plan → implement 推进）
...
- 世界/屏幕/视口坐标转换可被拖入、缩放、框选、overview 复用。
- 示例图正确显示；传入自定义节点绘制函数后，节点视觉可被替换。
- 10000 节点不创建 10000 个 DOM 节点；缩放和平移连续，无明显跳动。
- 容器 resize 后 Canvas 尺寸与内容同步，高清屏（DPR）下不模糊。
- 从资源树拖入后 graph 增加节点。

### 第二阶段验收

- 5 个及以下相邻兄弟不合并，6 个及以上自动合并为分组框。
