...
- **已完成切片**：
  ...
  - 视口平移缩放 `viewport.js` / `Minimap.vue` 受控 viewport + wheel zoom + blank pan + 测试（[plan](docs/superpowers/plans/2026-06-19-phase-3-viewport-pan-zoom.md)，commit `29c8ccb..3979f6b`，`npm test` 168 全过，`npm run build` 通过；dev server 可访问，Browser 插件无可用 `iab`，以组件真实事件验收覆盖交互）
